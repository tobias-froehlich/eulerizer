...
class Gui(tk.Frame):
    ...
    def __make_regions(self):
        ...
        for i in range(self.__const["EULER_PEDALS"]):
            ...
            (x8, y8) = self.__euler_to_coords(
                i+3.15,
                1.25,
                0
            )

            (x9, y9) = self.__euler_to_coords(
                i+3.45,
                0.7,
                0
            )

            (x10, y10) = self.__euler_to_coords(
                i+2.45,
                0.7,
                0
            )

            (x11, y11) = self.__euler_to_coords(
                i+2.9,
                -0.25,
                0
            )
    ...
    def __make_dots(self):
        ...
        for k in range(self.__const["EULER_LAYERS"]):
            ...
            for j in range(self.__const["EULER_ROWS"]):
                ...
                for i in range(self.__const["EULER_PEDALS"] + 2):
